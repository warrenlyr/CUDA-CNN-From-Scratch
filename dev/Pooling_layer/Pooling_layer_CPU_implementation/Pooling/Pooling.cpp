/*
* Convolutional layer & Pooling layer CPU implementation
* Author: Warren Liu, Yuan Ma
* Date: 3/13/2023
...
int main()
{
	...
	// Pooling
	cat_images = getFiles(CATS_PATH_OUTPUT);

	start = high_resolution_clock::now();

	for (int i = 0; i < cat_images.size(); i++) {

		Mat new_image = pool2D_max(cat_images[i].string());

		// TEST USE: write final images to output folder
		bool success = imwrite(string(CATS_PATH_FINAL) + cat_images[i].filename().string(), new_image);
		cout << "Success: " << success << endl;
	}
